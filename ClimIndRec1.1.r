...
### Create foldw following blockstyle or blindstyle approach ###
        if (!blockstyle_cv) {
...
### Loop to calculate scores ###
        ...
        for (r in 1:R){
            ...
            if(sum(is.na(tsamp))==0 && sum(is.na(tnsamp))==0){
                ...
                if (sum(inds)>1){
                    ...
                    ridge.cv=cv.glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),type.measure='mse',nfold=K_cv,alpha=0)

                    best_ridge_coef <- as.numeric(coef(ridge.cv, s = ridge.cv$lambda.min))[-1]

                    alasso.cv=cv.glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),type.measure="mse",nfold=K_cv,alpha=1,penalty.factor=1/abs(best_ridge_coef),keep=T)

                    best_alasso_coef <- coef(alasso.cv, s = alasso.cv$lambda.min)

                    alasso=glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),alpha=1,penalty.factor=1/abs(best_ridge_coef),lambda=alasso.cv$lambda.min)

                    pred=as.vector(predict(alasso,as.matrix(Xtest),s=alasso.cv$lambda.min))
        ...
        ridge.cv=cv.glmnet(x=as.matrix(X),y=as.matrix(Y),type.measure='mse',nfold=K_cv,alpha=0)
        print('?')
        best_ridge_coef <- as.numeric(coef(ridge.cv, s = ridge.cv$lambda.min))[-1]

        alasso.cv=cv.glmnet(x=as.matrix(X),y=as.matrix(Y),type.measure="mse",nfold=K_cv,alpha=1,penalty.factor=1/abs(best_ridge_coef),keep=T)

        best_alasso_coef <- coef(alasso.cv, s = alasso.cv$lambda.min)

        alasso=glmnet(x=as.matrix(X),y=as.matrix(Y),alpha=1,penalty.factor=1/abs(best_ridge_coef),lambda=alasso.cv$lambda.min)

        pred_fin=as.vector(predict(alasso,as.matrix(datas),s=alasso.cv$lambda.min))
...
### Loop to calculate scores ###
 	    ...
 	    inds=c(F)
            ...
            if(sum(is.na(tsamp))==0 && sum(is.na(tnsamp))==0){
                ...
                if	  (sum(inds)>1){
                    ...
                    lasso.cv=cv.glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),type.measure='mse',nfold=K_cv,alpha=1)
                    
                    best_lasso_coef <- as.numeric(coef(lasso.cv, s = lasso.cv$lambda.min))[-1]
                    if (sum(best_lasso_coef==0)>0){
                        Xtrain=Xtrain[,-which(best_lasso_coef==0)]
                        Xtest=Xtest[,-which(best_lasso_coef==0)]
                        datas=datas[,-which(best_lasso_coef==0)]
                    }
                    if (sum(best_lasso_coef!=0)!=0){
                        ridge.cv=cv.glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),type.measure='mse',nfold=K_cv,alpha=0)

                        ridge=glmnet(x=as.matrix(Xtrain),y=as.matrix(Ytrain),alpha=0,lambda=ridge.cv$lambda.min)

                        pred=as.vector(predict(ridge,as.matrix(Xtest),s=ridge.cv$lamda.min))
        ...
        lasso.cv=cv.glmnet(x=as.matrix(X),y=as.matrix(Y),type.measure='mse',nfold=K_cv,alpha=1)

        best_lasso_coef <- as.numeric(coef(lasso.cv, s = lasso.cv$lambda.min))[-1]

        if (sum(best_lasso_coef==0)>0){
            X=X[,-which(best_lasso_coef==0)]
            datas=datas[,-which(best_lasso_coef==0)]
        }

        ridge.cv=cv.glmnet(x=as.matrix(X),y=as.matrix(Y),type.measure='mse',nfold=K_cv,alpha=0)

        ridge=glmnet(x=as.matrix(X),y=as.matrix(Y),alpha=0,lambda=ridge.cv$lambda.min)

        pred_fin=as.vector(predict(ridge,as.matrix(datas),s=ridge.cv$lamda.min))
